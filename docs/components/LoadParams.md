...
- 过滤规则与 SearchParams 一致：先按前缀过滤，再按 `[start,end)` 区间过滤——前缀命中名称为 `前缀+纯数字后缀` 时按后缀**数值**过滤（前导零不影响，如 `[1,500001)` 命中 `multi_tenant_1000_0000001 ~ multi_tenant_1000_0500000`）；否则按名称排序后取下标切片，用于多 client 物理分割（如 client0 取 `[0,334)`、client1 取 `[334,668)`）
