...
- **passRate 口径**：pass = 请求无异常即成功，不要求融合后返回满 topK（含 sparse/BM25 子请求时返回不满 topK 属正常）。请求抛异常记为失败（内部以 -1 哨兵与"0 命中"区分），Total 日志会打印 avg hit count 供核对命中情况。
- **异常容错**：请求异常时记录失败并继续跑（与 SearchComp 一致），不会因单次错误中断整个组件。
