...
df = pd.read_csv(WS / "guanghong_300735_price.csv")
...
ax1.set_title("光弘科技（300735.SZ）近一年走势（前复权）", fontsize=14)
ax1.set_ylabel("价格（元）")
...
out = WS / "guanghong_300735_trend.png"
...
print(f"区间最低收盘: {lo['close'].item():.2f} 元 ({lo['time'].date()})" if hasattr(lo['close'],'item') else f"区间最低收盘: {lo['close']:.2f} 元 ({lo['time'].date()})")
print(f"saved: {out}")
